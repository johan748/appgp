...
const DirectorCreateGroupView: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Create Auth User (Supabase Auth) first to get the UUID
            ...
            try {
                ...
                const authResponse = await backend.createAuthUser(formData.email, formData.password, userMetadata);
            ...
            // 3. Create the User record in the database using the same ID
            const newUserData: Omit<User, 'id'> & { id?: string } = {
                id: authUserId, // Sync with Auth ID
