...
const EditMemberView: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (member) {
            const isBaptizedNow = formData.isBaptized === 'Sí';
            const prevIsBaptized = member.isBaptized;

            let updatedFriendProgress = { ...(member.friendProgress || {}) };

            if (isBaptizedNow && !prevIsBaptized) {
                // Now baptized, set date if not present
                if (!updatedFriendProgress.baptizedDate) {
                    updatedFriendProgress.baptizedDate = new Date().toISOString().split('T')[0];
                }
            } else if (!isBaptizedNow && prevIsBaptized) {
                // No longer baptized, remove date
                delete updatedFriendProgress.baptizedDate;
            }

            const updatedMember: Member = {
                ...member,
                ...formData,
                isBaptized: isBaptizedNow,
                gender: formData.gender as 'M' | 'F',
                role: formData.role as 'MIEMBRO' | 'LIDER' | 'LIDER_EN_FORMACION' | 'SECRETARIO',
                friendProgress: updatedFriendProgress
            };
