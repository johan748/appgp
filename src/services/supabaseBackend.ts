...
// Secondary client for Auth (no persistence) to prevent session hijacking
...
class SupabaseBackendService implements IBackendService {
    // Auth methods
    ...
    // Auth Automation - Fix applied to use client-side signUp
    ...
    async updateMember(member: Member): Promise<void> {
        console.log('UPDATING MEMBER:', member.firstName, member.lastName, 'isBaptized:', member.isBaptized);
        try {
            const { error } = await supabase
                .from('members')
                .update({
    ...
    async getStudents(): Promise<any[]> {
        try {
            ...
            return data.map(s => ({
                id: s.id,
                missionaryPairId: s.missionary_pair_id,
    ...
    async getStudentsByGP(gpId: string): Promise<any[]> {
        try {
            // First get all missionary pairs for this GP
            const { data: pairs, error: pairsError } = await supabase
                .from('missionary_pairs')
                .select('id')
                .eq('gp_id', gpId);
            ...
            // Then get students for these pairs
            const { data, error } = await supabase
                .from('bible_students')
                .select('*')
                .in('missionary_pair_id', pairIds)
                .order('created_at', { ascending: false });

            if (error) {
                console.error('Error fetching students by GP:', error);
                return [];
            }
    ...
    async addStudent(student: any): Promise<any> {
        try {
            const { data, error } = await supabase
                ...
                .insert([{
                    id: student.id,
                    missionary_pair_id: student.missionaryPairId,
                    ...
                    total_lessons: student.totalLessons,
            ...
            return {
                id: data.id,
                missionaryPairId: data.missionary_pair_id,
                firstName: data.first_name,
                lastName: data.last_name,
                cedula: data.cedula,
                birthDate: data.birth_date,
    ...
    async updateStudent(student: any): Promise<void> {
        try {
            const { error } = await supabase
                .from('bible_students')
                .update({
    ...
    async saveLesson(lesson: any): Promise<any> {
        try {
            const { data, error } = await supabase
                .from('bible_study_lessons')
                .upsert([{
                    ...
                    completion_date: lesson.completionDate
                }], {
