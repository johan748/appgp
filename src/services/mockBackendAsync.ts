...
import { User, SmallGroup, Member, Church, District, Zone, Association, WeeklyReport, MissionaryPair, Union, Student, BibleStudyLesson } from '../types';
...
export class MockBackendAsyncService implements IBackendService {
    ...
    async getStudents(): Promise<Student[]> {
        return Promise.resolve(mockBackend.getStudents());
    }

    async getStudentsByGP(gpId: string): Promise<Student[]> {
        return Promise.resolve(mockBackend.getStudentsByGP(gpId));
    }

    async addStudent(student: Student): Promise<Student> {
        return Promise.resolve(mockBackend.addStudent(student));
    }

    async updateStudent(student: Student): Promise<void> {
        return Promise.resolve(mockBackend.updateStudent(student));
    }

    async deleteStudent(id: string): Promise<void> {
        return Promise.resolve(mockBackend.deleteStudent(id));
    }

    async getLessons(): Promise<BibleStudyLesson[]> {
        return Promise.resolve(mockBackend.getLessons());
    }

    async saveLesson(lesson: BibleStudyLesson): Promise<BibleStudyLesson> {
        return Promise.resolve(mockBackend.saveLesson(lesson));
    }
